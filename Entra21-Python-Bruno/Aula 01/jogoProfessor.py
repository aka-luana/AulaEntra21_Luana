...
palavra, chances, tentativa_palavra, controleJogo = "", 0, "", True

def carregarConfig():
    global palavra, chances
    arqConfig = open("config.txt", "r")
    for linha in arqConfig: newLine = (linha.strip()).split(';')
    palavra, chances = newLine[0], int(newLine[1])

def limpaTela():
    if name == 'nt': system('cls') # for windows 
    else: system('clear') # for mac and linux(here, os.name is 'posix') 
...
def jogar():
    ...
    while controleJogo:
        ...
        print(*letras_usadas, "\n") # imprime item por item do array
        for x in palavra: tentativa_palavra += x if x in letras_usadas else "_"
        print(f"{tentativa_palavra}\n")
        ...
        if(controleJogo):
            ...
            while(True):
                if(ord(chute) >= 97 and ord(chute) <= 122): break
                else: chute = input("Caracter inválido digite novamente: ")

            while(True):
                if chute in letras_usadas: chute = input("Letra já utilizada. Digite outra: ")
                else:
                    letras_usadas.append(chute)
                    tentativaCont += 1 # coloquei isso
                    break

            if not chute in palavra: chances -= 1
        else:
            controleJogo = True
            break
...
def menu():
    ...
    while(escolhaJogador != 3):
        ...
        if(escolhaJogador == 1): jogar()
        elif(escolhaJogador == 2):
            modificaConfig(), limpaTela()
